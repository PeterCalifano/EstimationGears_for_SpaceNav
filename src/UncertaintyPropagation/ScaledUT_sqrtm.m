function [xhat_UT, P_UT] = SigmaPointTransform_sqrtm(nlinfun, xhat, Pcov, k, alpha, beta) %#codegen
%% PROTOTYPE
...
%  04-07-2023   Pietro Califano    sqrtm() replaced by chol() for square 
%                                  root covariance matrix computation
% -------------------------------------------------------------------------------------------------------------
%% DEPENDENCIES
% [-]
% -------------------------------------------------------------------------------------------------------------
...
[xhat_UT, P_UT] = ComputeCsiMeanCov(csiNext, weights);
    ...
    function [csi, weights] = GenerateSigmaPoints(xhat, P, npoints, k, alpha, beta)
        ...
        % Compute square root of matrix using Cholesky factorization
        S = sqrtm(lambda*P);
