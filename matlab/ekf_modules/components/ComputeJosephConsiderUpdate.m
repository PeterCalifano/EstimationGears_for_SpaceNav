function [dxErrorState, dStateCovPost, bUpdateAccepted, ...
        ...
        ComputeJosephConsiderUpdate(dStateCovPrior, ...
                                    dMeasurementResidual, ...
                                    dMeasurementCov, ...
                                    dStateObservationMatrix, ...
                                    dMeasurementUnderweightCoeff, ...
                                    bConsiderStateMask, ...
                                    bEnableRejection, ...
                                    dSquaredMahalanobisThreshold) %#codegen
%% SIGNATURE
...
if size(dStateCovPrior, 2) ~= ui32StateCount || ...
        any(~isfinite(dStateCovPrior), 'all') || ...
        norm(dStateCovPrior - transpose(dStateCovPrior), 'fro') > ...
        dPriorCovSymTolerance
...
if ui32MeasurementCount == 0 || ...
        ~isequal(size(dMeasurementCov), ...
                 [ui32MeasurementCount, ui32MeasurementCount]) || ...
        ~isequal(size(dStateObservationMatrix), ...
                 [ui32MeasurementCount, ui32StateCount]) || ...
        numel(bConsiderStateMask) ~= ui32StateCount || ...
        any(~isfinite(dMeasurementResidual)) || ...
        any(~isfinite(dMeasurementCov), 'all') || ...
...
if ~isfinite(dMeasurementUnderweightCoeff) || ...
        dMeasurementUnderweightCoeff < 0.0 || ...
        ~isfinite(dSquaredMahalanobisThreshold) || ...
        dSquaredMahalanobisThreshold <= 0.0
...
if norm(dMeasurementCov - transpose(dMeasurementCov), 'fro') > ...
        dMeasurementCovSymTolerance
    error('ComputeJosephConsiderUpdate:InvalidMeasurement', ...
...
[dInnovationCholFactor, dInnovationCovCholStatus] = ...
    chol(dInnovationCov, 'lower');
...
dSquaredMahalanobisDistance = ...
    transpose(dWhitenedResidual) * dWhitenedResidual;
...
if bEnableRejection && ...
        dSquaredMahalanobisDistance >= dSquaredMahalanobisThreshold
...
dStateMeasurementCrossCov = ...
    dStateCovPrior * transpose(dStateObservationMatrix);
dKalmanGain = transpose(dInnovationCholFactor' \ ...
    (dInnovationCholFactor \ transpose(dStateMeasurementCrossCov)));
...
dJosephStateTransform = eye(ui32StateCount) - ...
