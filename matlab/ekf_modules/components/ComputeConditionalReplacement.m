function [dxErrorState, dStateCovPost] = ...
        ComputeConditionalReplacement(dStateCovPrior, ...
                                      ui16ReplacementStateIdx, ...
                                      dReplacementResidual, ...
                                      dReplacementCov, ...
                                      bConsiderStateMask) %#codegen
%% SIGNATURE
% [dxErrorState, dStateCovPost] = ...
%     ComputeConditionalReplacement(dStateCovPrior, ...
%                                   ui16ReplacementStateIdx, ...
%                                   dReplacementResidual, ...
%                                   dReplacementCov, ...
...
if ui32ReplacementStateCount == 0 || ...
        any(ui16ReplacementStateIdx < uint16(1)) || ...
        ...
        ~isequal(size(dReplacementCov), ...
                 [ui32ReplacementStateCount, ui32ReplacementStateCount]) || ...
        numel(bConsiderStateMask) ~= ui32StateCount || ...
        any(~isfinite(dReplacementResidual)) || ...
        any(~isfinite(dReplacementCov), 'all')
...
if norm(dReplacementCov - transpose(dReplacementCov), 'fro') > ...
        dReplacementCovSymTolerance
    error('ComputeConditionalReplacement:InvalidReplacement', ...
...
dReplacementPriorCov = dStateCovPrior(ui16ReplacementStateIdx, ...
                                       ui16ReplacementStateIdx);
dReplacementRetainedCrossCov = dStateCovPrior( ...
    ui16ReplacementStateIdx, bRetainedStateMask);

% Factor the replacement-state prior covariance P_aa for stable solves.
[dReplacementPriorCholFactor, dReplacementPriorCholStatus] = ...
    chol(dReplacementPriorCov, 'lower');
...
dRetainedGivenReplacementMap = transpose( ...
    dReplacementPriorCholFactor' \ ...
...
dReplacementCovInjectionMap = zeros( ...
    ui32StateCount, ui32ReplacementStateCount);
dReplacementCovInjectionMap(ui16ReplacementStateIdx, :) = ...
    eye(ui32ReplacementStateCount);
dReplacementCovInjectionMap(bEstimableRetainedStateMask, :) = ...
    dEstimableGivenReplacementMap;

dConditionalPriorCov = dPriorConditioningTransform * dStateCovPrior * ...
...
if any(bEstimableRetainedStateMask)
    ...
    dSchurConsistencyErrorNorm = norm( ...
        dConditionalPriorCov( ...
            bEstimableRetainedStateMask, bRetainedStateMask) - ...
        dRetainedConditionalCov( ...
            bEstimableRetainedStateMask, bRetainedStateMask), ...
        'fro');
