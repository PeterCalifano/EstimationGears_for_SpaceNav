"""Public Python package entrypoint for EstimationGears_for_SpaceNav."""
...
def _export_wrapper_module(module_: ModuleType) -> None:
    ...
    Args:
        module_: Loaded extension module whose public names are exported.

    Example:
        from types import ModuleType
        module_ = ModuleType("example")
        module_.answer = 42
        module_.__all__ = ["answer"]
        _export_wrapper_module(module_)
        print(answer)
...
def _import_build_linked_wrapper() -> ModuleType:
    ...
    Example:
        module_ = _import_build_linked_wrapper()
        print(module_.__name__.endswith("EstimationGears_for_SpaceNav"))
