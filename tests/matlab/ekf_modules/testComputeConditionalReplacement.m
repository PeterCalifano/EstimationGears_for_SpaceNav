...
[dxErrorState, dStateCovPost] = ComputeConditionalReplacement(dStateCovPrior, ui16ReplacementStateIdx, ...
    dReplacementResidual, dReplacementCov, bConsiderStateMask);
[dxExpectedErrorState, dExpectedStateCovPost] = ComputeConditionalReference_(dStateCovPrior, ...
    ui16ReplacementStateIdx, dReplacementResidual, dReplacementCov, bConsiderStateMask);
...
verifyGreaterThan(testCase, norm(dStateCovPost(7:end, 1:6), 'fro'), 1.0e-6);
end
...
dReplacementCov = dStateCovPrior(ui16ReplacementStateIdx, ui16ReplacementStateIdx);

[dxErrorState, dStateCovPost] = ComputeConditionalReplacement(dStateCovPrior, ui16ReplacementStateIdx, ...
    zeros(3, 1), dReplacementCov, false(10, 1));
...
for ui32CaseIdx = 1:numel(cellReplacementStateIdx)
    ...
    [~, dStateCovPost] = ComputeConditionalReplacement(dStateCovPrior, ui16ReplacementStateIdx, ...
        [0.1; 0.2; -0.3], dReplacementCov, false(11, 1));
    dRetainedConditionalCov = SchurMarginalization(dStateCovPrior, bReplacementStateMask);
    dRetainedGivenReplacementMap = dStateCovPrior(bRetainedStateMask, bReplacementStateMask) / ...
        dStateCovPrior(bReplacementStateMask, bReplacementStateMask);
    dRecoveredRetainedCondCov = dStateCovPost(bRetainedStateMask, bRetainedStateMask) - ...
    ...
    verifyEqual(testCase, dRecoveredRetainedCondCov, ...
        dRetainedConditionalCov(bRetainedStateMask, bRetainedStateMask), 'AbsTol', 5.0e-13);
end
end
...
bReplacementStateMask(ui16ReplacementStateIdx) = true;
bEstimableRetainedStateMask = ~bReplacementStateMask & ~bConsiderStateMask;
...
verifyEqual(testCase, dStateCovPost(bReplacementStateMask, bConsiderStateMask), zeros(2, 2), 'AbsTol', 2.0e-14);
verifyGreaterThan(testCase, norm(dStateCovPost(bEstimableRetainedStateMask, bConsiderStateMask), 'fro'), 1.0e-6);
verifyEqual(testCase, chol(dStateCovPost, 'lower') * ...
    transpose(chol(dStateCovPost, 'lower')), dStateCovPost, 'AbsTol', 5.0e-13);
end
...
verifyError(testCase, @() ComputeConditionalReplacement(dStateCovPrior, uint16([2, 3]), ...
    [0.1; 0.2], eye(2), bConsiderStateMask), ...
...
verifyError(testCase, @() ComputeConditionalReplacement(dNonSymmetricPriorCov, uint16([1, 2]), ...
    zeros(2, 1), eye(2), false(6, 1)), ...
    'ComputeConditionalReplacement:InvalidPriorCovariance');
verifyError(testCase, @() ComputeConditionalReplacement(dIndefinitePriorCov, uint16([1, 2]), ...
    zeros(2, 1), eye(2), false(6, 1)), ...
    'ComputeConditionalReplacement:PriorCovarianceNotPositiveDefinite');
verifyError(testCase, @() ComputeConditionalReplacement(dStateCovPrior, uint16([1, 1]), ...
    zeros(2, 1), eye(2), false(6, 1)), ...
    'ComputeConditionalReplacement:InvalidReplacement');
verifyError(testCase, @() ComputeConditionalReplacement(dStateCovPrior, uint16([1, 2]), ...
    zeros(3, 1), eye(2), false(6, 1)), ...
    'ComputeConditionalReplacement:InvalidReplacement');
verifyError(testCase, @() ComputeConditionalReplacement(dStateCovPrior, uint16([1, 2]), zeros(2, 1), ...
    [1.0, 2.0; 2.0, 1.0], false(6, 1)), ...
    'ComputeConditionalReplacement:ReplacementCovarianceNotPositiveDefinite');
end

function [dxErrorState, dStateCovPost] = ComputeConditionalReference_(dStateCovPrior, ...
    ui16ReplacementStateIdx, dReplacementResidual, dReplacementCov, bConsiderStateMask)
...
dReplacementCovInjectionMap = zeros(ui32StateCount, numel(ui16ReplacementStateIdx));
dReplacementCovInjectionMap(bReplacementStateMask, :) = eye(numel(ui16ReplacementStateIdx));
dReplacementCovInjectionMap(bEstimableRetainedStateMask, :) = dEstimableGivenReplacementMap;

dStateCovPost = dPriorConditioningTransform * dStateCovPrior * transpose(dPriorConditioningTransform) + ...
...
for ui32RowIdx = 2:ui32StateCount
    for ui32ColumnIdx = 1:(ui32RowIdx - 1)
        dStateCovRoot(ui32RowIdx, ui32ColumnIdx) = 0.025 * sin(double(3 * ui32RowIdx + 2 * ui32ColumnIdx));
    end
end
