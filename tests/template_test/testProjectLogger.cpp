/// @file testProjectLogger.cpp
/// @brief Verifies the dependency-free EstimationGears logger contract.
...
TEST_CASE("project logger reads levels from the selected environment variable", "[logging]")
{
    ...
    constexpr const char *charTestVariableName_ = "ESTIMATION_GEARS_TEST_LOG_LEVEL";
...
TEST_CASE("project logger uses the EstimationGears environment variable by default", "[logging]")
{
    using namespace estimation_gears::logging;

    CEnvironmentVariableGuard objEnvironmentGuard_("ESTIMATION_GEARS_LOG_LEVEL");
    std::ostringstream objOutputStream_;
    std::ostringstream objDiagnosticStream_;
    CLogger objLogger_("component", ELogLevel::Error, ELogColorMode::Disabled, objOutputStream_,
                       objDiagnosticStream_);
